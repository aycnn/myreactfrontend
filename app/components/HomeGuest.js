    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value.length > 30) {
          draft.username.hasErrors = true
          draft.username.message = "username must be 30 max!"
        }
        return
      case "usernameAfterDelay":
        if (draft.username.value.length < 3) {
          draft.username.hasErrors = true
          draft.username.message = "username must be 3 min!"
        }
      ...
      case "usernameUniqueResults":
        if (action.value) {
          ...
          draft.username.message = "already taken"
        } else {
      ...
      case "emailAfterDelay":
        if (!/^\S+@\S+$/.test(draft.email.value)) {
          ...
          draft.email.message = "you must provide an email address"
        }
      ...
      case "emailUniqueResults":
        if (action.value) {
          ...
          draft.email.message = "already being used"
        } else {
      ...
      case "passwordAfterDelay":
        if (draft.password.value.length < 3) {
          draft.password.hasErrors = true
          draft.password.message = "Password must be minimum 3 characters!"
        }
        return
      case "submitForm":
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          appDispatch({ type: "flashMessages", value: "congrats!!!" })
        } catch (e) {
          console.log("olmuyor zanim")
        }
      }
